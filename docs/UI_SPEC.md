...
**顶部导航栏结构（自左至右）：**
1. **第一行（全局区）**：内容最大宽度 `topbar-primary-max-width`（1400px），左侧为品牌区（「Trading Review」衬线字标 34px），右侧为「账本切换器」「设置」「上传文件」「导入历史」全局操作按钮

**账本切换器（全局组件）：**
- 位置：第一行右侧、其余全局操作按钮左侧；形态为文字下拉触发器：`[当前账本名] + 类型徽标(实盘/模拟) + lucide chevron-down(16px)`
- 文字 14px，`color-text-secondary`；徽标用既有 chip 规范：实盘=`color-primary` 描边、模拟=`color-text-muted` 描边，圆角 `radius-pill`，不引入新色
- 下拉面板（`color-surface` 白底 + `color-border-subtle` 描边 + `shadow-sm`）：列出全部账本，当前项左侧 `lucide check`；底部「+ 新建账本」「账本管理」入口（跳设置页 §10.1b）
