...
| 2026-05-30 | 变更 | 总览净值图改版为「收益率走势」：纵轴百分比、0% 居中（`#d1cfc5` 实线基线），区间起点归一为 0%；新增多基准对比线配色（上证 `#a07e5a`、沪深300 `#5e5d59`、创业板指 `#9a7aa0`、科创50 `#2a9fbf`，账户保持 `#c96442`）与底部基准 chips；时间区间 pills + 筛选、收益率口径下拉沿用现有 surface/border/primary token；绩效卡「已清仓已实现盈亏」→「总盈亏」 | 用户要求按券商收益率页排版（保留品牌色）并支持日期筛选、卡片改总盈亏 | 6.x 图表色系、绩效摘要卡片 |
...
| 2026-06-03 | 修正 | 收益率走势基准保留上证综指、沪深300、创业板指、科创50；移除深证成指；底部基准 chip 使用已计算的百分数值直接展示，避免二次乘以 100 | 用户确认新的对比基准，并指出图表 tooltip 正常但 chip 收益率显示被放大 | 6.x 图表色系、收益率走势卡片 |
